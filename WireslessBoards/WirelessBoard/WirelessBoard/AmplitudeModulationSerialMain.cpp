...
void updatePhasesAndAmplitudes(AsierInhoSerial::AsierInhoSerialBoard* driver, float* phases, float* amplitudes) {
	unsigned char* message = driver->discretize(phases, amplitudes);
	driver->updateMessage(message);
}

void amplitudeModulation(AsierInhoSerial::AsierInhoSerialBoard* driver, float* phases, float* amplitudes, float* amplitudesOff, int initialFrequency = 50) {
...
void levitateScreen(AsierInhoSerial::AsierInhoSerialBoard* driver, int numTransducers, float* t_positions, float signature = 0) {
